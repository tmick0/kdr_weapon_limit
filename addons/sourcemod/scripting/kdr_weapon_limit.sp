...
public OnPluginStart()
{
    ...
    "Version of 'KDR Weapon Limit'", FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DONTRECORD)), OnVersionChanged);
...
public OnClientPostAdminCheck(client)
{
    ...
    if (MaintainRestrictions && !RestrictionExpempted[client])
    {
        ...
        if (GetTrieArray(h_Trie, authString, restrictionInfo, sizeof(Restrictions))) // We found the steam ID in the Trie
        {
            BombExplodePoints[client] = restrictionInfo.explode;
            DefuseBombPoints[client] = restrictionInfo.defuse;                
            SavedFrags[client] = restrictionInfo.frags;
            SavedDeaths[client] = restrictionInfo.deaths;
            BotKills[client] = restrictionInfo.botKills;
            ...
            if (restrictionInfo.isRestricted == 1) // Client was restricted when they left.
            {
...
public OnClientDisconnect(client)
{
    ...
    if (Enabled && IsClientInGame(client))
    {
        ...
        if (MaintainRestrictions)
        {
            ...
            restrictionInfo.isRestricted = Restricted[client];
            restrictionInfo.defuse = DefuseBombPoints[client];
            restrictionInfo.explode = BombExplodePoints[client];
            
            restrictionInfo.frags = GetClientFrags(client);
            restrictionInfo.deaths = GetClientDeaths(client);
            restrictionInfo.botKills = BotKills[client];
            
            SetTrieArray(h_Trie, authString, restrictionInfo, sizeof(Restrictions), true);
